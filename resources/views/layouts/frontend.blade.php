...
<html dir="{{ config("app.settings.direction") }}" class="{{ config("app.settings.direction") }}" lang="{{ str_replace("_", "-", app()->getLocale()) }}">
    <head>
        ...
        <style>
            ...
            /* FastOTP: Faster spinner animation (0.6s instead of default 2s) */
            .fa-spin-fast {
                -webkit-animation: fa-spin .6s infinite linear;
                animation: fa-spin .6s infinite linear;
            }
            /* FastOTP: Force pause-spinner to override fa-spin-fast shorthand */
            .pause-spinner {
        ...
        <script>
            document.addEventListener('livewire:init', () => {
                Livewire.on('stopLoader', () => {
                    setTimeout(() => {
                        document.querySelectorAll('#refresh, .fa-sync-alt').forEach(el => el.classList.add('pause-spinner'));
                    }, 500);
                });
            });

            // ── FastOTP: Smart Adaptive Polling ──────────────────────────────────
            // Active tab  : poll every config fetch_seconds (minimum 5s)
            // Hidden tab  : poll every SLOW_INTERVAL ms (30 seconds) to save server
            // Return focus: immediately fetch so user never waits
            // ─────────────────────────────────────────────────────────────────────
            // Admin setting (seconds) converted to ms, with a safe 5-second floor
            let   adminSeconds    = parseInt('{{ config("app.settings.fetch_seconds") }}');
            if (isNaN(adminSeconds) || adminSeconds < 5) adminSeconds = 5;
            
            const FAST_INTERVAL   = adminSeconds * 1000;
            const SLOW_INTERVAL   = 30000;  // 30 seconds – background tab
            ...
            function _doFetch() {
                ...
                _isFetching = true;
                document.querySelectorAll('#refresh, .fa-sync-alt').forEach(el => el.classList.remove('pause-spinner'));
                Livewire.dispatch('fetchMessages');
                // Livewire stopLoader event resets spinner; mark not-fetching after 8s safety
                setTimeout(() => { _isFetching = false; }, 8000);
                _scheduleFetch();
            }

            function _scheduleFetch() {
                clearTimeout(_fetchTimer);
                const delay = document.hidden ? SLOW_INTERVAL : FAST_INTERVAL;
                _fetchTimer = setTimeout(_doFetch, delay);
            }

            // When user comes back to the tab, fetch immediately
            ...
            // Livewire signals fetch is done → reset lock so next poll can run
            document.addEventListener('livewire:init', () => {
                Livewire.on('stopLoader', () => {
                    _isFetching = false;
                });
            ...
            // Kick off first scheduled poll (initial fetch already fires on DOMContentLoaded above)
            _scheduleFetch();
            // ─────────────────────────────────────────────────────────────────────
        </script>

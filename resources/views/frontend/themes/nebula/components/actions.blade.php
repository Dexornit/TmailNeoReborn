    ...
    <div>
            ...
            <form wire:submit.prevent="create" method="post">
                <div class="max-w-screen-lg mx-auto flex flex-col gap-2">
                    @if (count($emails) > 0 && $in_app)
                        <a href="{{ Util::localizeRoute("mailbox") }}" class="self-start appearance-none rounded-md py-3 px-5 bg-white dark:bg-gray-800 bg-opacity-25 dark:bg-opacity-40 text-white dark:text-gray-100 text-sm cursor-pointer focus:outline-none"><i class="fas fa-angle-double-left"></i></a>
                    @endif

                    <div class="w-full bg-white dark:bg-gray-800 bg-opacity-20 dark:bg-opacity-40 rounded-md">
                        <input class="appearance-none bg-transparent w-full border-0 rounded-md py-4 px-5 focus:outline-none placeholder-white dark:placeholder-gray-400 placeholder-opacity-30 text-white dark:text-gray-100" type="text" name="user" wire:model.defer="user" placeholder="{{ __("Enter Username") }}" />
                    </div>

                    <div class="flex items-stretch">
                        <div class="relative flex-1 bg-white dark:bg-gray-800 bg-opacity-20 dark:bg-opacity-40 rounded-l-md">
                            <x-dropdown width="full">
                                <x-slot name="trigger">
                                    <input x-ref="domain" type="text" class="block appearance-none bg-transparent border-0 w-full h-full py-4 px-5 pr-8 cursor-pointer focus:outline-none select-none rounded-l-md placeholder-white dark:placeholder-gray-400 placeholder-opacity-30 text-white dark:text-gray-100" placeholder="{{ __("Select Domain") }}" name="domain" wire:model="domain" readonly />
                                </x-slot>
                                <x-slot name="content">
        ...
        <div x-show.transition.in="!in_app" class="in-app-actions mt-4 px-8" style="display: none">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 max-w-screen-lg mx-auto">
                ...
                <div onclick="
                    this.querySelector('#refresh').classList.remove('pause-spinner');
                    if(typeof _isFetching !== 'undefined') _isFetching = false;
                    Livewire.dispatch('fetchMessages');
                " class="bg-white dark:bg-gray-800 bg-opacity-10 dark:bg-opacity-20 text-white dark:text-gray-100 rounded-md py-5 text-center hover:bg-opacity-25 dark:hover:bg-opacity-40 cursor-pointer flex justify-center items-center space-x-2">
